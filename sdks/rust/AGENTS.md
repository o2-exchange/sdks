...
| `withdraw(owner, session, asset_id, amount, to)` | owner, session, asset, amount, `impl IntoWithdrawDestination` | `Result<WithdrawResponse>` | Withdraw funds (`None`/`Some(&str)` or `Identity`) |
