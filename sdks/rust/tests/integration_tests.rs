...
#[tokio::test]
#[serial]
async fn test_withdraw_to_address_and_contract_id() {
    let shared = get_shared_setup().await;
    let mut client = O2Client::new(Network::Testnet);
    let markets = client.get_markets().await.unwrap();
    let asset = &markets[0].base;

    let before = client
        .api
        .get_balance(
            asset.asset.as_str(),
            Some(shared.maker_trade_account_id.as_str()),
            None,
        )
        .await
        .unwrap()
        .trading_account_balance;
    assert!(before >= 2, "insufficient faucet balance for withdrawals");

    let session = Session {
        owner_address: shared.maker_wallet.b256_address,
        session_private_key: [0; 32],
        session_address: [0; 32],
        trade_account_id: shared.maker_trade_account_id.clone(),
    ...
    let address_result = client
        .withdraw(&shared.maker_wallet, &session, &asset.asset, "1", None)
        .await
        .unwrap();
    assert!(address_result.tx_id.is_some(), "address withdrawal failed");

    let contract_result = client
        .withdraw(
            &shared.maker_wallet,
            &session,
            &asset.asset,
            "1",
            Identity::ContractId(shared.taker_trade_account_id.to_string()),
        )
        .await
        .unwrap();
    assert!(
        contract_result.tx_id.is_some(),
        "ContractId withdrawal failed"
    );
...
fn min_quantity_for_min_order_rounds_after_margin_to_step() {
    ...
    assert_eq!(market.scale_quantity(&quantity).unwrap(), 2_000_000);
